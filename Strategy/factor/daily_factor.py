...
本文件集中维护所有仅依赖 ``Daily_data/*.pkl`` 日频输入即可计算的因子逻辑，包含：

- 大批量 panel 方式的日频因子库
- ``DailyFactorLibraryAdapter`` 的统一加载 / 分块 / 落盘流程
- 基于 ``FactorRegistry`` 的纯日频自定义因子

不包含任何直接读取 ``min_data/*.fea`` 的分钟因子逻辑；此类逻辑后续单独维护。
"""
...
class DailyFactorLibraryAdapter:
    ...
    def __init__(
        ...
        self.loader = DailyDataLoader(daily_data_dir or config.DAILY_DATA_DIR)
    ...
    def _load_tables(self, as_of_date=None) -> Dict[str, pd.DataFrame]:
        tables: Dict[str, pd.DataFrame] = {}
        for key, col_name in tqdm(self.FIELD_MAP.items(), desc="① 宽表字段(pkl)", unit="field"):
            try:
                wide = self.loader.load_field(key, as_of_date)
                ...
                if key in self.REQUIRED_KEYS:
                    raise RuntimeError(f"读取 {key}.pkl 失败: {exc}") from exc
                logger.warning("读取 %s.pkl 失败: %s", key, exc)
                continue
    ...
    def compute_and_save_all(
        ...
        tables = self._load_tables(as_of_date)
        ...
        if start_date is not None or end_date is not None:
            ...
            if idx0 < 0 or idx1 < 0 or idx0 > idx1:
                raise ValueError("无法对齐 start/end 到本地交易日索引")
        ...
        try:
            with tqdm(total=n_out, desc="② 分块向量化(输出日进度)", unit="day") as pbar:
                for ci, cs in enumerate(chunk_starts):
                    ...
                    chunk_tables = {
                        col: wide.reindex(chunk_dates) for col, wide in tables.items()
                    }
                    ...
                    mask = (result_df["date"] >= d0) & (result_df["date"] <= d1)
            ...
            saved: Dict[str, Path] = {}
            with tqdm(factor_cols_ref, desc="③ 落盘因子列", unit="col") as pbar3:
                for fname in pbar3:
                    pbar3.set_postfix_str(
                        fname[:36] + ("…" if len(fname) > 36 else ""),
                        refresh=True,
                    )
                    ...
                    try:
                        ...
                        saved[fname] = save_wide_table(combined, out_path)
                    except Exception as exc:
                        logger.warning("保存因子 [%s] 失败: %s", fname, exc)
...
class MomentumFactor(FactorBase):
    ...
    def compute(self, daily_data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        return (close / close.shift(20) - 1).shift(1)
...
class CR3Factor(FactorBase):
    ...
    def compute(self, daily_data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        cr3 = up.rolling(26, min_periods=1).sum() / (
            dn.rolling(26, min_periods=1).sum() + _eps
        ) * 100
        return cr3.shift(1)
...
class BollPositionNormFactor(FactorBase):
    ...
    def compute(self, daily_data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        window = 20
        k = 2
        mid = close.rolling(window, min_periods=max(1, window // 2)).mean()
        std = close.rolling(window, min_periods=max(1, window // 2)).std()
        upper = mid + k * std
        lower = mid - k * std
        boll_range = upper - lower
        return ((close - lower) / boll_range.replace(0, np.nan)).shift(1)
