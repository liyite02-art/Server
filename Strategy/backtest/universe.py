...
def st_mask(
    ...
    if st_status is None:
        return pd.DataFrame(True, index=dates, columns=columns)
    st = standardize_wide(st_status).reindex(index=dates, columns=columns)
    return st.fillna(0).eq(0)
...
def build_universe_filter(
    ...
    st_ok = st_mask(dates, columns, st_status) if exclude_st else pd.DataFrame(True, index=dates, columns=columns)
    universe = age_ok & st_ok

    report = pd.DataFrame(index=dates)
    report.index.name = "TRADE_DATE"
    report["excluded_new_stock"] = (~age_ok).sum(axis=1)
    report["excluded_st"] = (age_ok & ~st_ok).sum(axis=1)
