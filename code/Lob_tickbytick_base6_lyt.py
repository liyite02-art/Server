...
def zscore_normalize(df):
    ...
    quantiles = (
        df.group_by("second")
        .agg(
            [pl.col(c).quantile(0.001).alias(f"{c}_low") for c in factor_cols] +
            [pl.col(c).quantile(0.999).alias(f"{c}_high") for c in factor_cols]
        )
    )
    df = df.join(quantiles, on="second", how="left")
    ...
    df = df.join(stats, on="second", how="left")
    ...
    return df.select(["code", "second"] + [c + "_dimless_cz" for c in factor_cols]).to_pandas()
...
def process_stock_group_vol(group):
    ...
    aligned_group['vol_to_bv_diff'] = np.where(aligned_group['bv_sum'].diff() != 0, aligned_group['vol']/np.abs(aligned_group['bv_sum'].diff() + eps), 0)
    aligned_group['vol_to_sv_diff'] = np.where(aligned_group['sv_sum'].diff() != 0, aligned_group['vol']/np.abs(aligned_group['sv_sum'].diff() + eps), 0)
...
def process_stock_group_osod(group):
    ...
    for w in [7,13,26]:
        aligned_group[f'br_{w}'] = np.where(aligned_group['short_strength'].rolling(w, min_periods=1).sum().abs() <= EPS,0,aligned_group['long_strength'].rolling(w, min_periods=1).sum() / aligned_group['short_strength'].rolling(w, min_periods=1).sum() * 100)
        aligned_group[f'ar_{w}'] = np.where(aligned_group['down_gravity'].rolling(w, min_periods=1).sum().abs() <= EPS,0,aligned_group['up_push'].rolling(w, min_periods=1).sum() / aligned_group['down_gravity'].rolling(w, min_periods=1).sum() * 100)
...
def process_stock_group_Kline(group):
    ...
    for w in [7,13,26]:
        aligned_group[f'PPR_{w}'] = np.where(aligned_group['down_penetration'].rolling(w,min_periods=1).mean().abs() <= EPS,0,aligned_group['up_penetration'].rolling(w,min_periods=1).mean() / aligned_group['down_penetration'].rolling(w,min_periods=1).mean())
...
main_logic_config = {
    ...
    'Slope': {
        ...
        'cols': ['code', 'time','bid_diff','ask_diff','time_diff_slope','std_bvp_slope','std_svp_slope'],
    },
    'Imbalance': {
        'func': process_stock_group_imbalance, 
        'cols': ['code', 'time', 'bid_ratio','pending_ratio_diff_1','pending_ratio_diff_2',
                'ratio_adjust_1','ratio_adjust_2'],
    },
    'Shape': {
    ...
    'Osod':{
        ...
        'cols':['code','time','K','D','J','bias_6','bias_12','bias_24','br_7','br_13','br_26',
                'ar_7','ar_13','ar_26','cci_5','cci_10','cci_20']
    },
    'Energy':{
        'func': process_stock_group_energy,
        'cols':['code','time', 'DIP', 'DIN', 'DX', 'ADX', 'ASI', 'avg_gain','avg_loss','RSI', 'RSII','MFI', 'VR']
    },
    'Terminal':{
        'func': process_stock_group_terminal,
        'cols': ['code', 'time', 'Boll_Width','Boll_Position', 'KC_Width', 'KC_Position',
                 'Mass', 'Donchian_Width', 'Donchian_Position','alpha_xsii','CR1', 'CR2', 'CR3', 'CR4']
    },
    'Kline':{
        'func': process_stock_group_Kline,
        'cols': ['code', 'time', 'macd_strength', 'macd_signal_gap','trend_momentum',
                 ...
                 'ret_ts_kurt_std', 'signed_vol', 'EPI','up_count', 'down_count',
                 'rvi', 'vol_parkinson', 'chaikin_vol']
    },
    'Klineshape':{
        'func': process_stock_group_Klineshape,
        'cols': ['code', 'time', 'close_location', 'close_loc_before', 'candle_ratio', 
                 'body_candle_ratio','Doji', 'overlap_ratio', 'candle_skew', 'gap_ratio',
                 'close_position', 'open_position', 'ma_bias', 'mdd', 'cum_mmd'],
    },
    'PCA':{
        'func': process_stock_group_pca,
        'cols': ['code', 'time', 'Order_bv_1', 'Order_bv_2', 'Order_sv_1', 'Order_sv_2', 'imb_1', 'imb_2',
                 'bid_diff_1', 'ask_diff_1', 'bid_diff_2', 'ask_diff_2'],
    },
    'PLS':{
...
if __name__ == '__main__':
    ...
    base_data_name = rf'Lob_tickbytick_base6_lyt'
    start_date = '20190104'
    end_date = '20251126'
